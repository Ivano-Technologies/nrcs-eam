...
4. **Magic links** point at **`/auth/verify`** on the SPA origin; after success, navigate to **`/app`**. Email base URL uses **`FRONTEND_ORIGIN`** on the API so email templates do not depend on `VITE_*` for links.
